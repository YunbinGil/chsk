...
#if UNITY_2022_3_OR_NEWER
                    _purchase = FindAnyObjectByType<chsk.Core.Services.KitchenManager>(FindObjectsInactive.Include) as IShopPurchaseProvider;
#else
                        _purchase = FindObjectOfType<chsk.Core.Services.KitchenManager>() as IShopPurchaseProvider;
#endif
                        break;
                    default: // Auto
#if UNITY_2022_3_OR_NEWER
                    _purchase = (IShopPurchaseProvider)FindAnyObjectByType<SpiceManager>(FindObjectsInactive.Include)
                             ?? FindAnyObjectByType<chsk.Core.Services.KitchenManager>(FindObjectsInactive.Include) as IShopPurchaseProvider;
#else
                        _purchase = (IShopPurchaseProvider)FindObjectOfType<SpiceManager>()
                                 ?? FindObjectOfType<chsk.Core.Services.KitchenManager>() as IShopPurchaseProvider;
#endif
                        break;
                }
            }
