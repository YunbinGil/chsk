            ...
            switch (providerType)
            {
...
#if UNITY_2022_3_OR_NEWER
                    _p = FindAnyObjectByType<Game.Kitchen.KitchenManager>(FindObjectsInactive.Include);
#else
                    _p = FindObjectOfType<Game.Kitchen.KitchenManager>();
#endif
                    break;
                ...
                default:
                    ...
                    _p = (IShopCatalogProvider)FindAnyObjectByType<SpiceManager>(FindObjectsInactive.Include)
                        ?? FindAnyObjectByType<Game.Kitchen.KitchenManager>(FindObjectsInactive.Include);
#else
                    _p = (IShopCatalogProvider)FindObjectOfType<SpiceManager>()
                        ?? FindObjectOfType<Game.Kitchen.KitchenManager>();
#endif
                    break;
            }
        }
